<?php
/** 一个示例tag，得到每个项目的nlp切出的tag词，标记每个示例tag在切出的tag数组中所在的位置，保存入库
 *
 */
...
require_once '/vendor/guzzle-http/vendor/autoload.php';

define('NLP_SERVICE_URL','http://localhost:8083/api/v1.0/nlp/cdd_web');

//读取有链接tag的白名单
$file = 'white_tags_link.txt';
$handler = fopen($file,'r'); //打开文件
$linkWhiteTagArr = [];
while(!feof($handler)){
    $lineData = fgets($handler,4096);
    $tagStr = trim($lineData);
    array_push($linkWhiteTagArr,$tagStr);
}
...
foreach($res as $value){
    ...
    $ID = $value->ID;
    $returnResArr = requestNlp($post_content);
    foreach($returnResArr as $site => $returnRes){
        if(!in_array($returnRes,$linkWhiteTagArr)){
            continue;
        }
        $insertArr[] = "(".$ID.",'".$returnRes."','".$post_title."',".$site.")";
        $insertCount =  $daoMysqli->insertToTagPage_Two($insertArr);
        if(!$insertCount) {
            echo 'insert error: tag为:'.$returnRes."   ID为:".$ID."   项目为:    ".$post_title."插入失败！\n";
        }else{
            $j += $insertCount;
            echo 'insert error: tag为:'.$returnRes."   ID为:".$ID."   项目为:    ".$post_title."插入ok！\n";
        }
        $insertArr = [];
    }
}
